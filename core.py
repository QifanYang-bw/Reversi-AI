...
class Reversi(object):
	...
	def __init__(self, chessMap = None, currentState = None, BlackCount = -1, WhiteCount = -1):
		if chessMap == None:
			...
			self.set_board_state(chessMap, currentState, BlackCount, WhiteCount)
	...
	def set_board_state(self, chessMap, currentState,
	...
	def __extend(self, pos_row, pos_col, self_state, oppo_state, xshift, yshift):
		...
		while flag:
			pos_row += xshift
			pos_col += yshift
			if position_test(pos_row, pos_col):
	...
	def __flip(self, pos_row, pos_col, self_state, oppo_state):
		...
		for (xshift, yshift) in pos_shift:
			(extend_succcess, extend_count) = __extend(pos_row, pos_col, self_state, oppo_state, xshift, yshift)
			if extent_success:
				new_row = pos_row
				new_col = pos_col
	...
	def validity_test(self, pos_row, pos_col, self_state, oppo_state):
		...
		if self.__chessMap[pos_col][pos_row] != BoardState.Empty:
	...
	def move(self, pos_row, pos_col, safety_check = True):
		if safety_check:
			(valid, error_description) = validity_test(self, pos_row, pos_col, self.__currentState, oppo_state)
			if not valid:
	...
	#----------Status Check----------#
	def check_availability(self, currentState = self.__currentState):
		#Could be optimized
		oppo_state = self.get_opponent_state()
		flag = False
		for row in range(n):
	...
	def check_winning_status(self):
		...
		if self.__BlackCount + self.__WhiteCount == n_squared:
			Finished = True
		if not self.check_availability() and not self.check_availability(self.get_opponent_state()):
			Finished = True
